...
{
  my_node_t * node = (my_node_t *) allocator->allocate(sizeof(my_node_t), allocator->state);
  if(node == NULL)
  {
    return NULL;
  }
  ...
  if(timer_num > 0)
  {
    node->count = (int64_t *) allocator->allocate(sizeof(int64_t)*timer_num, allocator->state);
    if (node->count == NULL)
    {
  ...
  if(pub_num > 0)
  {
    node->publisher = (rcl_publisher_t *) allocator->allocate(sizeof(rcl_publisher_t)*pub_num, allocator->state);
    ...
    node->callback = (callback_t *) allocator->allocate(sizeof(callback_t)*pub_num, allocator->state);
  ...
  if(sub_num > 0)
  {
    node->subscriber = (rcl_subscription_t *) allocator->allocate(sizeof(rcl_subscription_t)*sub_num, allocator->state);
  ...
  if(timer_num > 0)
  {
    node->timer = (rcl_timer_t *) allocator->allocate(sizeof(rcl_timer_t)*timer_num, allocator->state);
    if(node->timer == NULL)
    {
      if (node->subscriber_callback != NULL)
      {
...
void destroy_node(my_node_t * node, const rcl_allocator_t * allocator)
{
  ...
  if(node->publisher != NULL)
  {
    for (int i = 0; i < node->pub_num; i++)
    {
      VOID_RCCHECK(rcl_publisher_fini(&node->publisher[i], &node->rcl_node));
  ...
  rclc_executor_semantics_t pub_semantics)
{
  ...
  busy_wait_random(min_run_time_ms, max_run_time_ms);
  RCSOFTCHECK(rcl_publish(&node->publisher[pub_index], &pub_msg, NULL));
  now = rclc_now(&support);
  printf("Timer %lu %ld %ld\n", (unsigned long) &node->timer[timer_index], pub_msg.frame_id, now);
}
  ...
  rclc_executor_semantics_t pub_semantics)
{
  ...
  if (pub_index >= 0)
  {
    RCSOFTCHECK(rcl_publish(&node->publisher[pub_index], msg, NULL));
    printf("Subscriber %lu %ld %ld\n", (unsigned long) &node->subscriber[sub_index], msg->frame_id, now);     
  }
}
/*
