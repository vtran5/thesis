...
void node1_timer1_callback(rcl_timer_t * timer, int64_t last_call_time)
{
  ...
  RCSOFTCHECK(rclc_publish(&node1.publisher[0], &pub_msg, NULL, LET));
...
void node2_subscriber1_callback(const void * msgin)
{
  ...
  now = rclc_now(&support);
  RCSOFTCHECK(rclc_publish(&node2.publisher[0], msg, NULL, LET));
  sprintf(temp, "Subscriber %lu %ld %ld\n", (unsigned long) &node2.subscriber[0], msg->frame_id, now);
  strcat(stat2,temp);
...
void node3_subscriber1_callback(const void * msgin)
{
  ...
  now = rclc_now(&support);
  RCSOFTCHECK(rclc_publish(&node3.publisher[0], msg, NULL, LET));
  sprintf(temp, "Subscriber %lu %ld %ld\n", (unsigned long) &node3.subscriber[0], msg->frame_id, now);
  strcat(stat3,temp);
...
int main(int argc, char const *argv[])
{
    ...
    {
      RCCHECK(rclc_publisher_init(&node1.publisher[i], &node1.rcl_node, my_type_support, topic_name[0], &profile));    
    }
    for (i = 0; i < NODE2_PUBLISHER_NUMBER; i++)
    {
      RCCHECK(rclc_publisher_init(&node2.publisher[i], &node2.rcl_node, my_type_support, topic_name[1], &profile));    
    }
    ...
    for (i = 0; i < NODE2_SUBSCRIBER_NUMBER; i++)
    {
      ...
      RCCHECK(rclc_subscription_init(&node2.subscriber[i], &node2.rcl_node, my_type_support, topic_name[0], &profile));      //printf("Created subscriber %s:\n", topic_name[i]);
    ...
    for (i = 0; i < NODE3_SUBSCRIBER_NUMBER; i++)
    {
      ...
      RCCHECK(rclc_subscription_init(&node3.subscriber[i], &node3.rcl_node, my_type_support, topic_name[0], &profile));      //printf("Created subscriber %s:\n", topic_name[i]);
      custom_interfaces__msg__Message__init(&node3.sub_msg[i]);
    }

    for (i = 0; i < NODE3_PUBLISHER_NUMBER; i++)
    {
      RCCHECK(rclc_publisher_init(&node3.publisher[i], &node3.rcl_node, my_type_support, topic_name[2], &profile));     
    }
    ...
    rcutils_time_point_value_t callback_let3 = RCUTILS_MS_TO_NS(80);
    rcutils_time_point_value_t callback_let4 = RCUTILS_MS_TO_NS(80);
    unsigned int num_handles = 1;
    
    const rcutils_time_point_value_t period1 = RCUTILS_MS_TO_NS(10);
    ...
    printf("Executor fini\n");
      ...
      for (i = 0; i < NODE1_PUBLISHER_NUMBER; i++)
      {
        RCCHECK(rclc_publisher_fini(&node1.publisher[i]));
      }
#endif
    ...
    for (i = 0; i < NODE2_PUBLISHER_NUMBER; i++)
    {
      RCCHECK(rclc_publisher_fini(&node2.publisher[i]));
    }
#endif
...
#endif
    printf("Node 3 subscriber fini\n");
#if (NODE3_PUBLISHER_NUMBER > 0)
    for (i = 0; i < NODE3_PUBLISHER_NUMBER; i++)
    {
      RCCHECK(rclc_publisher_fini(&node3.publisher[i]));
    }
#endif
    printf("Node 3 publisher fini\n");
