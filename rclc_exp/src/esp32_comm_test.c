...
void node1_timer1_callback(rcl_timer_t * timer, int64_t last_call_time)
{
  ...
  int timer_index = 0;
  int pub_index = 0;
  timer_callback_print(node1, timer_index, pub_index, 1, 2, semantics);
}
...
void node1_subscriber1_callback(const void * msgin)
{
  ...
  subscriber_callback_print(node1, msg, sub_index, pub_index, min_run_time_ms, max_run_time_ms, pub_semantics);
}
