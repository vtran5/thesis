...
/*
 * This is the example for trigger condition ALL impact
 */
class Node1 : public MyNode
{
public:
	Node1() : MyNode("node1"), count1(0), count2(0), first_run(1)
	{
   		// Initialize the vector with 4 vectors of size 100 filled with 0's.
    timestamp = std::vector<std::vector<int64_t>>(2, std::vector<int64_t>(200, 0));
    ...
    timer2 = this->create_wall_timer(100ms, std::bind(&Node1::timer2_callback, this));
		timer1 = this->create_wall_timer(500ms, std::bind(&Node1::timer1_callback, this));
	}
...
private:
	...
	void timer2_callback()
	{
		...
		message.frame_id = count2++;
		message.stamp = now.nanoseconds();
		timestamp.at(1).at(message.frame_id) = (now - start_time).nanoseconds()/1000;
		//RCLCPP_INFO(this->get_logger(), "Publishing: '%d' at %ld", message.frame_id, timestamp.at(1).at(message.frame_id));
...
{
public:
	Node2() : MyNode("node2"), sub1_new_data(false), first_run(1)
	{
   		// Initialize the vector with 4 vectors of size 100 filled with 0's.
    timestamp = std::vector<std::vector<int64_t>>(2, std::vector<int64_t>(100, 0));
		...
		subscriber2 = this->create_subscription<custom_interfaces::msg::Message>(
			"topic2", 10, std::bind(&Node2::sub2_callback, this, _1));
	}
	...
	void sub1_callback(const custom_interfaces::msg::Message & msg)
  	{
    	...
    	busy_wait_random(100ms,150ms);
  	}

  void sub2_callback(const custom_interfaces::msg::Message & msg)
  	{
  		if (!sub1_new_data) // ignore if not received new topic1 message
  		{
  			timestamp.at(1).at(msg.frame_id) = 1;
  			return;
  		}
  		rclcpp::Time now = this->now();
  		timestamp.at(1).at(msg.frame_id) = (now.nanoseconds() - msg.stamp)/1000;
  		sub1_new_data = false;
    	//RCLCPP_INFO(this->get_logger(), "I heard: '%d' at %ld", msg.frame_id, timestamp.at(1).at(msg.frame_id));
    	//msg.header.stamp = now - start_time;
    	busy_wait_random(100ms,150ms);
  	}
...
int main(int argc, char const *argv[])
{
   ...
   auto thread2 = std::thread([&]() 
	...
	ret = configure_thread(thread2, ThreadPriority::HIGH, 1);
	if (!ret) {
