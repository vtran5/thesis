...
/*
 * This is the example for fix execution order
 */
class Node1 : public MyNode
{
public:
	Node1() : MyNode("node1"), count1(0), first_run(1)
	{
   		// Initialize the vector with 4 vectors of size 100 filled with 0's.
    timestamp = std::vector<std::vector<int64_t>>(1, std::vector<int64_t>(200, 0));
...
public:
	...
	{
   		// Initialize the vector with 4 vectors of size 100 filled with 0's.
    timestamp = std::vector<std::vector<int64_t>>(3, std::vector<int64_t>(100, 0));

		subscriber1 = this->create_subscription<custom_interfaces::msg::Message>(
			"topic1", 10, std::bind(&Node2::sub1_callback, this, _1));

		subscriber3 = this->create_subscription<custom_interfaces::msg::Message>(
			"topic1", 10, std::bind(&Node2::sub3_callback, this, _1));

		subscriber2 = this->create_subscription<custom_interfaces::msg::Message>(
			"topic1", 10, std::bind(&Node2::sub2_callback, this, _1));
	}
...
private:
	...
	void sub2_callback(const custom_interfaces::msg::Message & msg)
 	...
 	void sub3_callback(const custom_interfaces::msg::Message & msg)
  	{
  		rclcpp::Time now = this->now();
	...
	rclcpp::Subscription<custom_interfaces::msg::Message>::SharedPtr subscriber3;
	rclcpp::Time start_time;
};
