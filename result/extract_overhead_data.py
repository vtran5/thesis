...
def extract_data_from_file(file_path):
    ...
    # Extracting the callbackLET
    callbackLET = content[0].split(":")[1].strip()
    
    # Extracting data for various sections
    input_overhead_with_let = extract_data_for_section(content, "Input Overhead with LET", "Input Overhead without LET")
    input_overhead_without_let = extract_data_for_section(content, "Input Overhead without LET", "Output Thread Overhead")
    output_thread_overhead_with_let = extract_data_for_section(content, "Output Thread Overhead", "input overhead increase total")
    input_overhead_increase = extract_table_data(content, "input overhead increase total")
    output_overhead_increase = extract_table_data(content, "output overhead increase total")
    
    return {
        "CallbackLET": callbackLET,
        "Input Overhead with LET": input_overhead_with_let,
        "Input Overhead without LET": input_overhead_without_let,
        "Output Thread Overhead with LET": output_thread_overhead_with_let,
        "Input Overhead Increase total": input_overhead_increase,
        "Output Overhead Increase total": output_overhead_increase
    }
...
if __name__ == "__main__":
    file_path = "./result/overhead_profile_varied_let.txt"  # You can replace with your file path
    ...
    for data in extracted_data:
        ...
        for data_type, data_values in data.items():
            if data_type not in ["CallbackLET", "input overhead increase total", "output overhead increase total"]:
