...
def count_timers_per_executor(json_file_path):
    # Load the JSON file
    with open(json_file_path, "r") as f:
        data = json.load(f)

    # Extract the number of timers for each executor
    timers_count_per_executor = {}
    for executor, entities in data["executors"].items():
        timers_count = sum(1 for entity in entities if "Timer" in entity)
        timers_count_per_executor[executor] = timers_count

    return timers_count_per_executor

def extract_data_for_section(content, start_section, end_section=None):
    """Extract data between start and end section markers."""
    data = {}
    in_section = False
    current_timersection = None
    executor_id = None
    ...
    for line in content:
        ...
        if in_section:
            if 'ExecutorID' in line:
                ...
                data[(current_timersection, executor_id)] = stats
            elif line in ["Without data", "With data"]:
                current_timersection = line
            else:
                break
    return data
...
def extract_all_data_from_content(file_path):
    ...
    """Extract data for all number of timerss from the content."""
    all_data = []
    current_data = {}
    ...
    # Append the last number of timers data
    if current_data:
        all_data.append(current_data)
...
def plot_box_whiskier(filtered_df, title):
    ...
    executors = sorted(filtered_df["ExecutorID"].unique())
    # Define positions for each number of timers
    ...
    for idx, executor in enumerate(executors):
        ...
        positions = sorted(executor_data["number of timers"].unique())
        for pos in positions:
            data_row = executor_data[executor_data["number of timers"] == pos]
        ...
        ax.set_xlabel('Number of timers')
...
def plot_total_overhead(df,title):
    ...
    for idx, executor in enumerate(executors):
        ...
        executor_data = df[df["ExecutorID"] == executor].sort_values(by="number of timers")
        ...
        # executor_data["number of timers"] = executor_data["number of timers"] .sort_values()
        index_positions = range(len(executor_data["number of timers"].unique()))    
        ...
        ax1.set_xticklabels(executor_data["number of timers"].unique())
        ax1.set_xlabel('Number of timers')
        ax1.set_ylabel('Overhead Total (%)')

        ax2.set_title(f'{executor}')
        ax2.set_xticks([i for i in index_positions])
        ax2.set_xticklabels(executor_data["number of timers"].unique())
        ax2.set_xlabel('Number of timers')
...
if __name__ == "__main__":
    ...
    for data in extracted_data:
        json_path = data["path"]
        timer_count = count_timers_per_executor(json_path)
        ...
        for data_type, data_values in data.items():
            if data_type not in ["path", "input overhead increase total", "output overhead increase total"]:
                for (data_availability, executor_id), stats in data_values.items():
                    row = {
                        "number of timers": timer_count[executor_id],
                        "data type": data_type,
