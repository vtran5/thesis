...
int main(int argc, char const *argv[])
{
    ...
    if (executor_period > 0)
    {
        ...
        thread_create(&thread3, policy, 49, 2, rclc_executor_spin_period_wrapper, &ex3);
        thread_create(&thread4, policy, 49, 3, rclc_executor_spin_period_wrapper, &ex4);
    }
    else
    {
        thread_create(&thread1, policy, 49, 0, rclc_executor_spin_wrapper, &executor1);
        thread_create(&thread1, policy, 49, 1, rclc_executor_spin_wrapper, &executor2);
        thread_create(&thread1, policy, 49, 2, rclc_executor_spin_wrapper, &executor3);
        thread_create(&thread1, policy, 49, 3, rclc_executor_spin_wrapper, &executor4);
    }
